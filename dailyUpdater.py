...
from get_winners import get_winners
from get_losers import get_losers
load_dotenv()
...
## Need to create a proper winners_helper
async def winners_helper() -> None:
    return await get_winners()

## Need to create a proper losers_helper
async def losers_helper() -> None:
    return await get_losers()
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    isRepeat = True
    application.job_queue.run_repeating(default_repeat, interval=10, first=3, chat_id=chat_id, name='default_update')
    ...
    '\n\nBy typing the command you have activate /start'\
    '\nyou activated the natrual feature of this bot that sends you daily updates on'\
    '\nthe top 100 winners and losers of the stock market, which will be sent everyday at 08:00H'\
    '\n\nTo turn off this feature just type the command /turnoffdefupdate'\
    '\n\nTo turn on this feature again, you could type /start command again or'\
    '\nyou can use the /turnondefupdate command'
...
async def help(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    "\nor to turn back on the default daily update feature of the top winners and losers"\
    "\nif it was previously turned off"\
    "\n/help is to request a description of the commands in this bot,"\
    "\n\n"\
    "\n/winners is to get the latest day top 100 winners from the stock market"\
    "\n/losers is to get the latest day top 100 losers from the stock market"\
    "\n\n"\
    ...
    "\n/turnoffdefupdate is to turn the daily default updates of top winners and losers"\
    '\nthat is sent every day at 08:00H'\
    "\n\n/turnondefupdate is to turn back on your default daily update of the top winners and losers"\
    ...
    "\n\n"\
    "\nIf you would like to only recieve your custom daily updates,"\
    "\nturn off updates and then re-set your requierd updates with the commands"\
    "\n\n"\
    "\nKeep in mind that by default the default daily updates will be automatically sent to you"\
...
async def winners(update: Update, context: ContextTypes.DEFAULT_TYPE):
    todays_winners: List[Dict] = await winners_helper()
    result: str = ''
    result_one: str = ''
    result_two: str = ''
    result_three: str = ''
    for index, item in enumerate(todays_winners):
        for key, value in item.items():
            result += f'{key}:{value}\n'
        result += '\n\n'
    result_one = result[0:4096]
    result_two = result[4096:8192]
    result_three = result[8192:12288]
    result_four = result[12288:16384]
    result_five = result[16384:20480]
    result_six = result[20480:]
    await context.bot.send_message(context._chat_id, 'Todays winners:::\n\n\n')
    await context.bot.send_message(context._chat_id, result_one)
    await context.bot.send_message(context._chat_id, result_two)
    await context.bot.send_message(context._chat_id, result_three)
    await context.bot.send_message(context._chat_id, result_four)
    await context.bot.send_message(context._chat_id, result_five)
    await context.bot.send_message(context._chat_id, result_six)

async def losers(update: Update, context: ContextTypes.DEFAULT_TYPE):
    todays_losers: List[Dict] = await losers_helper()
    result: str = ''
    result_one: str = ''
    result_two: str = ''
    result_three: str = ''
    for index, item in enumerate(todays_losers):
        for key, value in item.items():
            result += f'{key}:{value}\n'
        result += '\n\n'
    result_one = result[0:4096]
    ...
    result_four = result[12288:16384]
    result_five = result[16384:20480]
    result_six = result[20480:24001]
    result_seven = result[24001:]
    await context.bot.send_message(context._chat_id, 'Todays losers:::\n\n\n')
    await context.bot.send_message(context._chat_id, result_one)
    await context.bot.send_message(context._chat_id, result_two)
    await context.bot.send_message(context._chat_id, result_three)
    await context.bot.send_message(context._chat_id, result_four)
    await context.bot.send_message(context._chat_id, result_five)
    await context.bot.send_message(context._chat_id, result_six)
    await context.bot.send_message(context._chat_id, result_seven)

async def turnoffdefupdate(update: Update, context: ContextTypes.DEFAULT_TYPE):
    global is_default_repeat
    if is_default_repeat:
        current_jobs = application.job_queue.get_jobs_by_name('default_update')
        for job in current_jobs:
            job.schedule_removal()
        is_default_repeat = False
        return await update.message.reply_text('Default updates have been turned off')
    else:
        return await update.message.reply_text('You currently do not have any updates to turn off at the moment :(')

## Change to daily once confident    
async def turnondefupdate(update: Update, context: ContextTypes.DEFAULT_TYPE):
    global is_default_repeat
    global chat_id
    is_default_repeat = True
    application.job_queue.run_repeating(default_repeat, interval=10, first=3, chat_id=chat_id, name='default_update')
    return await update.message.reply_text('Your default daily updates of top winners and losers has been turned on :)')
...
async def default_repeat(context: ContextTypes.DEFAULT_TYPE):
    todays_winners: List[Dict] = await winners_helper()
    result: str = ''
    result_one: str = ''
    result_two: str = ''
...
if __name__ == '__main__':
    ...
    application.add_handler(CommandHandler('turnoffdefupdate', turnoffdefupdate))
    application.add_handler(CommandHandler('turnondefupdate', turnondefupdate))
